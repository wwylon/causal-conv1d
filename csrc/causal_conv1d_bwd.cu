...
template <int kNThreads_, int kWidth_, bool kSiluAct_, bool kIsVecLoad_,
...
struct Causal_conv1d_bwd_kernel_traits {
  ...
  static constexpr int kSmemExchangeSize =
      kNThreads * kNBytes * kNElts * (!kSiluAct ? 1 : kNExchangeRounds + 1);
  static constexpr int kSmemSize =
      custom_max({kSmemExchangeSize,
                  int(sizeof(typename BlockReduceFloatT::TempStorage))}) +
      (kIsVecLoad ? 0 : kSmemIOSize);
};

template <typename Ktraits, bool kDeterministic>
__global__ __launch_bounds__(Ktraits::kNThreads) void causal_conv1d_bwd_kernel(
  ...
  vec_t *smem_exchange_x =
      reinterpret_cast<vec_t *>(smem_ + Ktraits::kSmemIOSize) +
      kNThreads * kNExchangeRounds;
  auto &smem_reduce_float =
      *reinterpret_cast<typename Ktraits::BlockReduceFloatT::TempStorage *>(
  ...
  input_t *x = reinterpret_cast<input_t *>(params.x_ptr) +
               batch_id * params.x_batch_stride + dim_id * params.x_c_stride;
  weight_t *weight = reinterpret_cast<weight_t *>(params.weight_ptr) +
                     dim_id * params.weight_c_stride;
  input_t *dout = reinterpret_cast<input_t *>(params.dout_ptr) +
                  batch_id * params.dout_batch_stride +
                  dim_id * params.dout_c_stride;
  input_t *dx = reinterpret_cast<input_t *>(params.dx_ptr) +
                batch_id * params.dx_batch_stride + dim_id * params.dx_c_stride;
  float *dweight = reinterpret_cast<float *>(params.dweight_ptr) +
                   dim_id * params.dweight_c_stride;
  float bias_val =
      params.bias_ptr == nullptr
          ? 0.f
          : float(reinterpret_cast<weight_t *>(params.bias_ptr)[dim_id]);

  // Thread kNThreads - 1 will load the first elements of the next chunk so we
...
#pragma unroll
  for (int i = 0; i < kWidth; ++i) {
    weight_vals[i] = weight[i * params.weight_width_stride];
  }
  ...
  for (int chunk = n_chunks - 1; chunk >= 0; --chunk) {
    ...
    if constexpr (kIsVecLoad) {
      ...
      typename Ktraits::BlockLoadVecT(smem_load_vec)
          .Load(reinterpret_cast<vec_t *>(dout),
    ...
    if constexpr (!kSiluAct) {
      ...
      __syncthreads();
      reinterpret_cast<vec_t *>(dout_vals_load)[1] =
          smem_exchange[tidx < kNThreads - 1 ? tidx + 1 : 0];
      __syncthreads();
...
#pragma unroll
      ...
      // Exchange the dout_vals. It's possible that we need to do 2 rounds of
      // exchange if input_t is 16 bits (since then we'd have 8 values of float)
      __syncthreads();
      // Thread 0 don't write yet, so that thread kNThreads - 1 can read
      // the first elements of the next chunk.
      if (tidx > 0) {
#pragma unroll
        for (int r = 0; r < kNExchangeRounds; ++r) {
          smem_exchange[r * kNThreads + tidx] =
              reinterpret_cast<vec_t *>(dout_vals)[r];
        }
      }
      __syncthreads();
#pragma unroll
      for (int r = 0; r < kNExchangeRounds; ++r) {
        reinterpret_cast<vec_t *>(dout_vals)[kNExchangeRounds + r] =
            smem_exchange[r * kNThreads +
                          (tidx < kNThreads - 1 ? tidx + 1 : 0)];
      }
      __syncthreads();
      // Now thread 0 can write the first elements of the current chunk.
      if (tidx == 0) {
...
#pragma unroll
  for (int w = 0; w < kWidth; ++w) {
    ...
    if (tidx == 0) {
      if constexpr (kDeterministic) {
        float *dweight_ws =
            reinterpret_cast<float *>(params.dweight_workspace_ptr);
        dweight_ws[batch_id * params.dweight_workspace_batch_stride +
                   dim_id * params.dweight_workspace_dim_stride + w] =
            dweight_vals[w];
      } else {
        atomicAdd(&reinterpret_cast<float *>(
                      dweight)[w * params.dweight_width_stride],
                  dweight_vals[w]);
      }
    }
  }
  if (params.bias_ptr != nullptr) {
    __syncthreads();
    dbias_val =
        typename Ktraits::BlockReduceFloatT(smem_reduce_float).Sum(dbias_val);
    if (tidx == 0) {
      if constexpr (kDeterministic) {
        float *dbias_ws = reinterpret_cast<float *>(params.dbias_workspace_ptr);
        dbias_ws[batch_id * params.dbias_workspace_batch_stride + dim_id] =
            dbias_val;
      } else {
        atomicAdd(&reinterpret_cast<float *>(params.dbias_ptr)[dim_id],
                  dbias_val);
      }
    }
  }
  ...
  BOOL_SWITCH(params.seqlen % kNElts == 0, kIsVecLoad, [&] {
    BOOL_SWITCH(params.silu_activation, kSiluAct, [&] {
      BOOL_SWITCH(params.deterministic, kDeterministic, [&] {
        using Ktraits =
            Causal_conv1d_bwd_kernel_traits<kNThreads, kWidth, kSiluAct,
                                            kIsVecLoad, input_t, weight_t>;
        constexpr int kSmemSize = Ktraits::kSmemSize;
        dim3 grid(params.batch, params.dim);
        auto kernel = &causal_conv1d_bwd_kernel<Ktraits, kDeterministic>;
        if (kSmemSize >= 48 * 1024) {
          // There is a slight signature discrepancy in HIP and CUDA
          // "FuncSetAttribute" function.
          C10_CUDA_CHECK(cudaFuncSetAttribute(
              ROCM_ONLY((void *)) kernel,
              cudaFuncAttributeMaxDynamicSharedMemorySize, kSmemSize));
          ROCM_ONLY(std::cerr
                    << "Warning (causal_conv1d bwd launch): attempting to set "
                       "maxDynamicSharedMemorySize on an AMD GPU which is "
                       "currently a non-op (in ROCm versions <= 6.1). This "
...
template <int kNThreads_, int kWidth_, int kChunkSizeL_, bool kSiluAct_,
          bool kIsVecLoad_, typename input_t_, typename weight_t_>
...
template <typename Ktraits, bool kHasSeqIdx, bool kHasDfinalStates,
          bool kDeterministic>
__global__
__launch_bounds__(Ktraits::kNThreads) void causal_conv1d_channellast_bwd_kernel(
    ConvParamsBwd params) {
  constexpr int kWidth = Ktraits::kWidth;
  constexpr int kNThreads = Ktraits::kNThreads;
  constexpr bool kSiluAct = Ktraits::kSiluAct;
  ...
  __shared__ input_t
      x_smem[kWidth - 1 + kChunkSizeL + kWidth - 1][kChunkSizeC + kNElts];
  ...
  input_t *dout = reinterpret_cast<input_t *>(params.dout_ptr) +
                  batch_id * params.dout_batch_stride +
                  (chunk_l_id * kChunkSizeL + l_idx) * params.dout_l_stride +
                  chunk_c_id * kChunkSizeC + c_idx * kNElts;
  input_t *dx = reinterpret_cast<input_t *>(params.dx_ptr) +
                batch_id * params.dx_batch_stride +
                (chunk_l_id * kChunkSizeL + l_idx) * params.dx_l_stride +
                chunk_c_id * kChunkSizeC + c_idx * kNElts;
  float *dweight = reinterpret_cast<float *>(params.dweight_ptr) +
  ...
  input_t *initial_states =
      params.initial_states_ptr == nullptr || chunk_l_id > 0
          ...
          : reinterpret_cast<input_t *>(params.initial_states_ptr) +
                ...
                chunk_c_id * kChunkSizeC + c_idx * kNElts;
  input_t *dinitial_states =
      params.dinitial_states_ptr == nullptr || chunk_l_id > 0
          ? nullptr
          : reinterpret_cast<input_t *>(params.dinitial_states_ptr) +
                batch_id * params.dinitial_states_batch_stride +
                l_idx * params.dinitial_states_l_stride +
                chunk_c_id * kChunkSizeC + c_idx * kNElts;
  input_t *dfinal_states =
      params.dfinal_states_ptr == nullptr
          ? nullptr
          : reinterpret_cast<input_t *>(params.dfinal_states_ptr) +
                batch_id * params.dfinal_states_batch_stride +
                chunk_c_id * kChunkSizeC;
...
#pragma unroll
  for (int l = 0; l < Ktraits::kNLoads; ++l) {
    ...
    if (chunk_l_id * kChunkSizeL + l * kLPerLoad + l_idx < params.seqlen &&
      ...
      reinterpret_cast<vec_t *>(dout_vals_load)[0] = *reinterpret_cast<vec_t *>(
          dout + l * kLPerLoad * params.dout_l_stride);
      reinterpret_cast<vec_t *>(x_vals_load)[0] =
          *reinterpret_cast<vec_t *>(x + l * kLPerLoad * params.x_l_stride);
    }
    reinterpret_cast<vec_t *>(dout_smem[l * kLPerLoad + l_idx])[c_idx] =
        reinterpret_cast<vec_t *>(dout_vals_load)[0];
    reinterpret_cast<vec_t *>(
        x_smem[kWidth - 1 + l * kLPerLoad + l_idx])[c_idx] =
        reinterpret_cast<vec_t *>(x_vals_load)[0];
  }
  // Load the elements from the previous chunk or next chunk that are needed for
  // convolution.
  if (l_idx < kWidth - 1) {
    input_t dout_vals_load[kNElts] = {0};
    input_t x_vals_load[kNElts] = {0};
    if ((chunk_l_id + 1) * kChunkSizeL + l_idx < params.seqlen &&
        chunk_c_id * kChunkSizeC + c_idx * kNElts < params.dim) {
      reinterpret_cast<vec_t *>(dout_vals_load)[0] =
          *reinterpret_cast<vec_t *>(dout + kChunkSizeL * params.dout_l_stride);
    }
    ...
    reinterpret_cast<vec_t *>(dout_smem[kChunkSizeL + l_idx])[c_idx] =
        reinterpret_cast<vec_t *>(dout_vals_load)[0];
    reinterpret_cast<vec_t *>(x_smem[l_idx])[c_idx] =
        reinterpret_cast<vec_t *>(x_vals_load)[0];
  }
  // Need to load (kWdith - 1) extra x's on the right to recompute the
  // (kChunkSizeL + kWidth - 1) outputs
  if constexpr (kSiluAct) {
    if (l_idx < kWidth - 1) {
      input_t x_vals_load[kNElts] = {0};
      if ((chunk_l_id + 1) * kChunkSizeL + l_idx < params.seqlen &&
          chunk_c_id * kChunkSizeC + c_idx * kNElts < params.dim) {
        reinterpret_cast<vec_t *>(x_vals_load)[0] =
            *reinterpret_cast<vec_t *>(x + kChunkSizeL * params.x_l_stride);
      }
      reinterpret_cast<vec_t *>(
          x_smem[kWidth - 1 + kChunkSizeL + l_idx])[c_idx] =
          reinterpret_cast<vec_t *>(x_vals_load)[0];
    }
  }
  ...
  constexpr int kLPerThread =
      constexpr_min(kChunkSizeL * kChunkSizeC / kNThreads, kChunkSizeL);
...
#pragma unroll
    for (int i = kWidth - 1 + kLPerThread;
         i < kWidth - 1 + kLPerThread + kWidth - 1; ++i) {
      x_vals[i] = float(x_smem[col_idx * kLPerThread + i][row_idx]);
    }
#pragma unroll
...
#pragma unroll
      for (int w = 0; w < kWidth; ++w) {
        if constexpr (!kHasSeqIdx) {
          ...
          out_val += seq_idx_thread[i + w] == seq_idx_cur
...
#pragma unroll
    for (int i = 0; i < kLPerThread; ++i) {
      if constexpr (!kHasSeqIdx) {
        ...
        dweight_vals[w] +=
            seq_idx_thread[i + w] == seq_idx_thread[kWidth - 1 + i]
                ? x_vals[i + w] * dout_vals[i]
                : 0.f;
      }
    }
    ...
    if (col_idx == 0 && chunk_c_id * kChunkSizeC + row_idx < params.dim) {
      if constexpr (kDeterministic) {
        float *dweight_ws =
            reinterpret_cast<float *>(params.dweight_workspace_ptr);
        dweight_ws[batch_id * params.dweight_workspace_batch_stride +
                   chunk_l_id * params.dweight_workspace_dim_stride *
                       params.dim +
                   (chunk_c_id * kChunkSizeC + row_idx) *
                       params.dweight_workspace_dim_stride +
                   w] = dweight_vals[w];
      } else {
        atomicAdd(&reinterpret_cast<float *>(
                      dweight)[row_idx * params.dweight_c_stride +
  ...
  if (params.bias_ptr != nullptr) {
    ...
    if (col_idx == 0 && chunk_c_id * kChunkSizeC + row_idx < params.dim) {
      if constexpr (kDeterministic) {
        ...
        atomicAdd(&reinterpret_cast<float *>(
                      params.dbias_ptr)[chunk_c_id * kChunkSizeC + row_idx],
                  dbias_val);
      }
    }
  }
...
#pragma unroll
    for (int w = 0; w < kWidth; ++w) {
      if constexpr (!kHasSeqIdx) {
        ...
        dx_vals[i] += seq_idx_thread[kWidth - 1 + i + w] == seq_idx_cur
                          ? weight_vals[kWidth - 1 - w] * dout_vals[i + w]
                          : 0.f;
      }
    }
    // if (dfinal_states != nullptr) {
    if constexpr (kHasDfinalStates) {
      if (chunk_l_id * kChunkSizeL + col_idx * kLPerThread + i >=
              params.seqlen - kWidth + 1 &&
          chunk_l_id * kChunkSizeL + col_idx * kLPerThread + i <
              params.seqlen &&
          chunk_c_id * kChunkSizeC + row_idx < params.dim) {
        dx_vals[i] +=
            float(dfinal_states[((chunk_l_id * kChunkSizeL +
                                  col_idx * kLPerThread + i) -
                                 (params.seqlen - kWidth + 1)) *
                                    params.dfinal_states_l_stride +
                                row_idx * params.dfinal_states_c_stride]);
      }
    }
  }

  float dxinit_vals[kWidth - 1] = {0};
  static_assert(kLPerThread >= kWidth - 1); // So only threads with col_idx == 0
                                            // need to handle dinitial_states
...
#pragma unroll
      ...
      if constexpr (kHasDfinalStates) {
        if (i >= params.seqlen) {
          dxinit_vals[i] +=
              float(dfinal_states[(i - params.seqlen) *
                                      params.dfinal_states_l_stride +
                                  row_idx * params.dfinal_states_c_stride]);
        }
      }
...
#pragma unroll
    for (int i = 0; i < kWidth - 1; ++i) {
      x_smem[i][row_idx] = dxinit_vals[i];
    }
  }
  __syncthreads();
...
#pragma unroll
  for (int l = 0; l < Ktraits::kNLoads; ++l) {
    input_t dx_vals_store[kNElts];
    reinterpret_cast<vec_t *>(dx_vals_store)[0] = reinterpret_cast<vec_t *>(
        x_smem[kWidth - 1 + l * kLPerLoad + l_idx])[c_idx];
    if (chunk_l_id * kChunkSizeL + l * kLPerLoad + l_idx < params.seqlen &&
        chunk_c_id * kChunkSizeC + c_idx * kNElts < params.dim) {
      *reinterpret_cast<vec_t *>(dx + l * kLPerLoad * params.dx_l_stride) =
          reinterpret_cast<vec_t *>(dx_vals_store)[0];
    }
  }
  if (dinitial_states != nullptr && l_idx < kWidth - 1 &&
      chunk_c_id * kChunkSizeC + c_idx * kNElts < params.dim) {
    input_t dxinit_vals_store[kNElts];
    reinterpret_cast<vec_t *>(dxinit_vals_store)[0] =
        reinterpret_cast<vec_t *>(x_smem[l_idx])[c_idx];
    *reinterpret_cast<vec_t *>(dinitial_states) =
        reinterpret_cast<vec_t *>(dxinit_vals_store)[0];
  }
}

template <int kNThreads, int kWidth, typename input_t, typename weight_t>
void causal_conv1d_channellast_bwd_launch(ConvParamsBwd &params,
                                          cudaStream_t stream) {
  BOOL_SWITCH(params.silu_activation, kSiluAct, [&] {
      ...
      BOOL_SWITCH(params.dfinal_states_ptr != nullptr, kHasDfinalStates, [&] {
        BOOL_SWITCH(params.deterministic, kDeterministic, [&] {
          BOOL_SWITCH(params.seqlen <= 128, kChunkSizeL64, [&] {
            // kChunkSizeL = 128 is slightly faster than 64 when seqlen is
            // larger
            static constexpr int kChunk = kChunkSizeL64 ? 64 : 128;
            using Ktraits = Causal_conv1d_channellast_bwd_kernel_traits<
                kNThreads, kWidth, kChunk, kSiluAct, true, input_t, weight_t>;
            // constexpr int kSmemSize = Ktraits::kSmemSize;
            constexpr int kChunkSizeL = Ktraits::kChunkSizeL;
            constexpr int kChunkSizeC = Ktraits::kNEltsPerRow;
            const int n_chunks_L =
                (params.seqlen + kChunkSizeL - 1) / kChunkSizeL;
            const int n_chunks_C = (params.dim + kChunkSizeC - 1) / kChunkSizeC;
            dim3 grid(params.batch, n_chunks_L, n_chunks_C);
            dim3 block(Ktraits::kNThreads);
            auto kernel = &causal_conv1d_channellast_bwd_kernel<
                Ktraits, kHasSeqIdx, kHasDfinalStates, kDeterministic>;
            // if (kSmemSize >= 48 * 1024) {
            //     C10_CUDA_CHECK(cudaFuncSetAttribute(
...
template <typename input_t, typename weight_t>
void causal_conv1d_channellast_bwd_cuda(ConvParamsBwd &params,
                                        cudaStream_t stream) {
  if (params.width == 2) {
    causal_conv1d_channellast_bwd_launch<128, 2, input_t, weight_t>(params,
                                                                    stream);
  } else if (params.width == 3) {
    causal_conv1d_channellast_bwd_launch<128, 3, input_t, weight_t>(params,
                                                                    stream);
  } else if (params.width == 4) {
    causal_conv1d_channellast_bwd_launch<128, 4, input_t, weight_t>(params,
                                                                    stream);
  }
}

template void causal_conv1d_bwd_cuda<float, float>(ConvParamsBwd &params,
                                                   cudaStream_t stream);
template void causal_conv1d_bwd_cuda<at::Half, float>(ConvParamsBwd &params,
                                                      cudaStream_t stream);
template void causal_conv1d_bwd_cuda<at::BFloat16, float>(ConvParamsBwd &params,
                                                          cudaStream_t stream);
template void causal_conv1d_bwd_cuda<float, at::Half>(ConvParamsBwd &params,
                                                      cudaStream_t stream);
template void causal_conv1d_bwd_cuda<at::Half, at::Half>(ConvParamsBwd &params,
                                                         cudaStream_t stream);
template void
causal_conv1d_bwd_cuda<at::BFloat16, at::Half>(ConvParamsBwd &params,
                                               cudaStream_t stream);
template void causal_conv1d_bwd_cuda<float, at::BFloat16>(ConvParamsBwd &params,
                                                          cudaStream_t stream);
template void
causal_conv1d_bwd_cuda<at::Half, at::BFloat16>(ConvParamsBwd &params,
                                               cudaStream_t stream);
template void
causal_conv1d_bwd_cuda<at::BFloat16, at::BFloat16>(ConvParamsBwd &params,
                                                   cudaStream_t stream);

template void
causal_conv1d_channellast_bwd_cuda<float, float>(ConvParamsBwd &params,
                                                 cudaStream_t stream);
template void
causal_conv1d_channellast_bwd_cuda<at::Half, float>(ConvParamsBwd &params,
                                                    cudaStream_t stream);
template void
causal_conv1d_channellast_bwd_cuda<at::BFloat16, float>(ConvParamsBwd &params,
                                                        cudaStream_t stream);
template void
causal_conv1d_channellast_bwd_cuda<float, at::Half>(ConvParamsBwd &params,
                                                    cudaStream_t stream);
template void
causal_conv1d_channellast_bwd_cuda<at::Half, at::Half>(ConvParamsBwd &params,
                                                       cudaStream_t stream);
template void causal_conv1d_channellast_bwd_cuda<at::BFloat16, at::Half>(
    ConvParamsBwd &params, cudaStream_t stream);
template void
causal_conv1d_channellast_bwd_cuda<float, at::BFloat16>(ConvParamsBwd &params,
                                                        cudaStream_t stream);
template void causal_conv1d_channellast_bwd_cuda<at::Half, at::BFloat16>(
    ConvParamsBwd &params, cudaStream_t stream);
template void causal_conv1d_channellast_bwd_cuda<at::BFloat16, at::BFloat16>(
    ConvParamsBwd &params, cudaStream_t stream);